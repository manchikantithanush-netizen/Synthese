...
class CycleEnergyCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final cardBgColor = isPeriod
        ? (isLightMode ? const Color(0xFFFDECEE) : const Color(0xFF2A1516)) // Soft red tint
        : (isLightMode ? const Color(0xFFF4F4F5) : const Color(0xFF151515)); // Default grey/black
        ...
        children: [
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              border: Border.all(color: thumbIconColor.withOpacity(0.2), width: 1.5),
            ),
            child: Row(
              children: [
                Container(
          ...
          Text("Current Cycle Day", style: TextStyle(color: subtitleColor, fontSize: 15, fontWeight: FontWeight.w500)),
          const SizedBox(height: 4),
          Row(
            crossAxisAlignment: CrossAxisAlignment.baseline,
            textBaseline: TextBaseline.alphabetic,
            children: [
              Text("$cycleDayToday", style: TextStyle(color: textColor, fontSize: 56, fontWeight: FontWeight.bold, letterSpacing: -2)),
              Text(" / $avgCycleLength", style: TextStyle(color: subtitleColor, fontSize: 24, fontWeight: FontWeight.w600)),
            ],
          ),
                ...
                child: Stack(
                  ...
                  children: [
                    Container(
                      width: maxWidth, 
                      height: 50, 
                      decoration: BoxDecoration(color: progressBarBg, borderRadius: BorderRadius.circular(16))
                    ),
                    Container(
                      width: activeWidth, 
                      height: 50,
                      decoration: BoxDecoration(
                        ...
                        borderRadius: BorderRadius.only(
                          ...
                          topRight: Radius.circular(progressRatio >= 1.0 ? 16 : 0),
                          bottomRight: Radius.circular(progressRatio >= 1.0 ? 16 : 0),
                        ),
                      ),
                    ),
                    ...
                    for (int day in loggedCycleDays)
                      if (day > 0 && day <= avgCycleLength)
                        Positioned(
                          left: ((day / avgCycleLength) * maxWidth).clamp(8.0, maxWidth - 8.0) - 1.5,
                          child: Container(
                            width: 3, 
                            height: 12, 
                            decoration: BoxDecoration(
          ...
          Row(
            children: [
              Container(width: 8, height: 8, decoration: const BoxDecoration(color: pinkColor, shape: BoxShape.circle)),
