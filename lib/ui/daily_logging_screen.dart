...
class _DailyLoggingScreenState extends State<DailyLoggingScreen>
  ...
  // Animation controller for logged animation
  ...
  final List<String> _mucusOptions = [
    'Dry',
    'Sticky',
    'Creamy',
    'Egg white',
  ...
  Future<void> _saveLog() async {
    ...
    try {
      ...
      final DateTime lastPeriodStart =
          (data['lastPeriodStart'] as Timestamp?)?.toDate() ?? DateTime.now();
      ...
      if ([
        ...
        if (cycleDay <= periodLength + 2) {
          isNewPeriod = false;
        } else if (cycleDay > periodLength + 4 &&
          ...
          if (isEarlyPeriod == true)
            isNewPeriod = true;
          else
            spottingOutsidePeriod = true;
      ...
      if (isNewPeriod && logDateOnly.isAfter(lastStartOnly)) {
        ...
        for (var doc in logsQuery.docs) {
          ...
          if (isBleedingPhase) {
            if (isBleeding &&
                (docData['spottingOutsidePeriod'] != true ||
                    date.difference(lastStartOnly).inDays < periodLength + 4)) {
              periodEndDate = date;
          ...
          if (!isBleedingPhase &&
              (f == 'Spotting' || docData['spottingOutsidePeriod'] == true))
            spottingDays++;
        }

        final int actualPeriodLength =
            periodEndDate!.difference(lastStartOnly).inDays + 1;
        final int actualCycleLength = logDateOnly
            .difference(lastStartOnly)
            .inDays
            .clamp(1, 60);

        final newCycleData = {
          'cycleLength': actualCycleLength,
          'periodLength': actualPeriodLength > 0
              ? actualPeriodLength
              : periodLength,
                  ...
                  children: [
                    ScaleTransition(
                      ...
                      child: Container(
                        ...
                        height: 100,
                        decoration: BoxDecoration(
                          color: const Color(0xFFEC548A).withOpacity(0.15),
