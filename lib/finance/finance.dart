...
class _FinancePageState extends State<FinancePage> {
  ...
  String _formatCurrency(double amount) {
    final formatter = NumberFormat.currency(symbol: _currencySymbol, decimalDigits: 2);
      ...
      child: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Total Balance Card
            _buildTotalBalanceCard(isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
            const SizedBox(height: 16),

            // Account Balances Row
            _buildAccountsRow(isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
            const SizedBox(height: 12),
              ...
              child: Align(
                ...
                child: CupertinoButton(
                  ...
                  child: Row(
                    ...
                    children: [
                      Icon(CupertinoIcons.arrow_right_arrow_left, size: 16, color: const Color(0xFF007AFF)),
                      const SizedBox(width: 6),
                      Text('Transfer', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: const Color(0xFF007AFF))),
                    ],
                  ),
                ),
              ),
            ...
            AnimatedOpacity(
              opacity: _isModalOpen ? 0.0 : 1.0,
              duration: const Duration(milliseconds: 150),
              child: PremiumButton(text: "Add Transaction", onPressed: _showAddTransactionModal),
            ),
            const SizedBox(height: 24),

            // Monthly Summary
            buildMonthlySummary(
              context: context, isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor,
              selectedMonth: _selectedMonth, transactionsStream: _transactionsStream, formatCurrency: _formatCurrency,
            ),
            const SizedBox(height: 16),
            ...
            buildCategoryPieChart(
              context: context, isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor,
              selectedMonth: _selectedMonth, transactionsStream: _transactionsStream, categoriesStream: _categoriesStream,
              categoriesMap: _categoriesMap, formatCurrency: _formatCurrency,
            ...
            // Search Bar
            _buildSearchBar(isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
            const SizedBox(height: 16),

            // Month Filter
            _buildMonthFilter(isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
            const SizedBox(height: 16),

            // Category Filter
            _buildCategoryFilter(isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
            const SizedBox(height: 24),
            ...
            // Transaction List
            _buildTransactionList(isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
          ],
        ),
      ),
    );
  ...
  Widget _buildTotalBalanceCard({required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              Text("Total Balance", style: TextStyle(color: subTextColor, fontSize: 14, fontWeight: FontWeight.w600)),
              const SizedBox(height: 12),
              Text(_formatCurrency(totalBalance), style: TextStyle(color: balanceColor, fontSize: 36, fontWeight: FontWeight.bold)),
            ],
          ),
        );
      },
  ...
  Widget _buildAccountsRow({required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    return StreamBuilder<QuerySnapshot>(
      stream: _accountsStream,
      builder: (context, snapshot) {
        if (!snapshot.hasData) {
          return Row(
            children: List.generate(3, (index) => Expanded(
              child: Padding(
                padding: EdgeInsets.only(right: index < 2 ? 8 : 0, left: index > 0 ? 8 : 0),
                child: Container(
                  padding: const EdgeInsets.all(16),
                  decoration: BoxDecoration(color: cardColor, borderRadius: BorderRadius.circular(20)),
                  child: Column(
                    children: [
                      Icon(CupertinoIcons.circle, color: subTextColor, size: 24),
                      const SizedBox(height: 8),
        ...
        final accounts = snapshot.data!.docs.map((doc) => Account.fromMap(doc.data() as Map<String, dynamic>)).toList();

        return Row(
          children: accounts.map((account) => Expanded(
            child: Padding(
              padding: EdgeInsets.only(right: account != accounts.last ? 8 : 0, left: account != accounts.first ? 8 : 0),
              child: _buildAccountCard(account: account, isDark: isDark, cardColor: cardColor, textColor: textColor, subTextColor: subTextColor),
            ),
          )).toList(),
        );
      },
  ...
  Widget _buildAccountCard({required Account account, required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    ...
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.center,
        children: [
          Icon(account.icon, color: isDark ? Colors.white70 : Colors.black54, size: 24),
          const SizedBox(height: 8),
          Text(account.name, style: TextStyle(color: subTextColor, fontSize: 12, fontWeight: FontWeight.w600)),
          const SizedBox(height: 4),
          FittedBox(
            fit: BoxFit.scaleDown,
            child: Text(_formatCurrency(account.balance), style: TextStyle(color: balanceColor, fontSize: 16, fontWeight: FontWeight.bold)),
          ),
        ],
      ),
    );
  ...
  Widget _buildTransactionList({required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    return StreamBuilder<QuerySnapshot>(
        ...
        return StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting && !snapshot.hasData) {
              return const Center(child: Padding(padding: EdgeInsets.all(40), child: CircularProgressIndicator(color: Color(0xFFEC548A))));
            }
            ...
            final filteredDocs = allDocs.where((doc) {
              ...
              if (transaction.date.year != _selectedMonth.year || transaction.date.month != _selectedMonth.month) return false;
              if (_selectedCategoryFilter != null && transaction.categoryId != _selectedCategoryFilter) return false;
              if (_searchQuery.isNotEmpty) {
                final query = _searchQuery.toLowerCase();
                final categoryName = category?.name.toLowerCase() ?? '';
                final note = transaction.note?.toLowerCase() ?? '';
                if (!categoryName.contains(query) && !note.contains(query)) return false;
              }
              return true;
            }).toList();
              ...
              return Container(
                ...
                child: Column(
                  children: [
                    ...
                    const SizedBox(height: 16),
                    Text(allDocs.isEmpty ? "No transactions yet" : "No matching transactions", style: TextStyle(color: subTextColor, fontSize: 16)),
                    const SizedBox(height: 8),
                    Text(allDocs.isEmpty ? "Tap 'Add Transaction' to get started" : "Try adjusting your filters", style: TextStyle(color: subTextColor.withOpacity(0.7), fontSize: 13)),
                  ],
                ),
              );
            }
            ...
            return Container(
              ...
              child: ListView.separated(
                ...
                separatorBuilder: (context, index) => Divider(color: isDark ? Colors.white12 : Colors.black12, height: 1, indent: 60),
                itemBuilder: (context, index) {
                  ...
                  final amountColor = isExpense ? const Color(0xFFFF3B30) : const Color(0xFF34C759);
                  ...
                  return Dismissible(
                    ...
                    onDismissed: (direction) => _deleteTransaction(doc.id, transaction),
                    child: ListTile(
                      contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
                      leading: Container(
                        width: 44, height: 44,
                        decoration: BoxDecoration(color: (category?.color ?? Colors.grey).withOpacity(0.15), borderRadius: BorderRadius.circular(12)),
                        child: Center(child: Icon(category?.icon ?? CupertinoIcons.question_circle, color: category?.color ?? Colors.grey, size: 22)),
                      ),
                      title: Text(category?.name ?? 'Unknown', style: TextStyle(color: textColor, fontSize: 15, fontWeight: FontWeight.w600)),
                      subtitle: Text(dateStr, style: TextStyle(color: subTextColor, fontSize: 12)),
                      trailing: Text('$amountPrefix${_formatCurrency(transaction.amount)}', style: TextStyle(color: amountColor, fontSize: 15, fontWeight: FontWeight.bold)),
                    ),
                  );
                },
              ),
  ...
  Widget _buildSearchBar({required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Icon(CupertinoIcons.search, color: subTextColor, size: 20),
          suffixIcon: _searchQuery.isNotEmpty
              ? GestureDetector(onTap: () => setState(() => _searchQuery = ''), child: Icon(CupertinoIcons.clear_circled_solid, color: subTextColor, size: 18))
              : null,
  ...
  Widget _buildMonthFilter({required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    final now = DateTime.now();
    final canGoForward = _selectedMonth.year < now.year || (_selectedMonth.year == now.year && _selectedMonth.month < now.month);
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          GestureDetector(
            onTap: () { HapticFeedback.lightImpact(); setState(() => _selectedMonth = DateTime(_selectedMonth.year, _selectedMonth.month - 1)); },
            child: Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: isDark ? Colors.white12 : Colors.black12, borderRadius: BorderRadius.circular(10)),
              child: Icon(CupertinoIcons.chevron_left, color: textColor, size: 18),
            ),
          ),
          Text(monthLabel, style: TextStyle(color: textColor, fontSize: 16, fontWeight: FontWeight.w600)),
          GestureDetector(
            onTap: canGoForward ? () { HapticFeedback.lightImpact(); setState(() => _selectedMonth = DateTime(_selectedMonth.year, _selectedMonth.month + 1)); } : null,
            child: Container(
              padding: const EdgeInsets.all(8),
              decoration: BoxDecoration(color: isDark ? Colors.white12 : Colors.black12, borderRadius: BorderRadius.circular(10)),
              child: Icon(CupertinoIcons.chevron_right, color: canGoForward ? textColor : subTextColor.withOpacity(0.3), size: 18),
            ),
          ),
        ],
      ),
  ...
  Widget _buildCategoryFilter({required bool isDark, required Color cardColor, required Color textColor, required Color subTextColor}) {
    return StreamBuilder<QuerySnapshot>(
      stream: _categoriesStream,
      builder: (context, snapshot) {
          ...
          child: Container(
            ...
            child: Row(
              children: [
                ...
                Icon(CupertinoIcons.chevron_down, color: subTextColor, size: 16),
              ],
            ),
          ),
        );
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: cardColor,
          borderRadius: BorderRadius.circular(16),
          border: isSelected ? Border.all(color: const Color(0xFF34C759), width: 2) : null,
        ),
        child: Row(
          children: [
            if (category != null) ...[
              ...
              Container(
                ...
                child: Icon(CupertinoIcons.square_grid_2x2, color: subTextColor, size: 20),
              ),
              const SizedBox(width: 14),
            ],
            Expanded(
            ...
            if (isSelected)
              Icon(CupertinoIcons.checkmark_circle_fill, color: const Color(0xFF34C759), size: 24),
          ],
        ),
      ),
    );
  ...
  Future<void> _deleteTransaction(String docId, FinanceTransaction transaction) async {
    final uid = FirebaseAuth.instance.currentUser?.uid;
    if (uid == null) return;
